...
import { DollarSign, FileText, CheckCircle, TrendingUp, Package } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const loadDashboardStats = async () => {
    try {
      ...
      const totalFaturado = orcamentos?.filter((orc) => orc.status === 'Aprovado').reduce((sum, orc) => sum + Number(orc.valor_final), 0) || 0;

      const orcamentosMes = orcamentos?.filter(
        (orc) => new Date(orc.created_at) >= mesInicio && new Date(orc.created_at) <= mesFim
      ).length || 0;

      const orcamentosAprovados = orcamentos?.filter((orc) => orc.status === 'Aprovado').length || 0;

      const granitoContagem: { [key: string]: number } = {};
      orcamentos?.forEach((orc) => {
        orc.itens_orcamento?.forEach((item: any) => {
          const nome = item.granitos?.nome || 'Desconhecido';
          granitoContagem[nome] = (granitoContagem[nome] || 0) + item.quantidade;
        });
      });

      const granitoMaisVendido = Object.entries(granitoContagem).sort((a, b) => b[1] - a[1])[0]?.[0] || 'N/A';

      const mesesFaturamento: { [key: string]: number } = {};
      orcamentos?.filter((orc) => orc.status === 'Aprovado').forEach((orc) => {
        const mes = format(new Date(orc.created_at), 'MMM/yy', { locale: ptBR });
        mesesFaturamento[mes] = (mesesFaturamento[mes] || 0) + Number(orc.valor_final);
      });
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm md:text-base text-zinc-400">Visão geral do seu negócio</p>
        </div>
              ...
              <div className="space-y-4">
                {stats.faturamentoMensal.map((item) => (
                  <div key={item.mes} className="flex items-center justify-between">
                    <span className="text-zinc-400 capitalize">{item.mes}</span>
                    <span className="text-white font-semibold">{formatCurrency(item.valor)}</span>
                  </div>
                ))}
              </div>
            ) : (
